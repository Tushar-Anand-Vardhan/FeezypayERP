"use client";

import Link from "next/link";
import { usePathname } from "next/navigation";
import { Can } from "@/lib/authz/can";
import type { AuthzBootstrap } from "@/lib/authz/bootstrap-shared";
import { PRINCIPAL_PORTAL_NAV } from "@/lib/principal-portal/nav";

type Props = {
  authz: AuthzBootstrap | null;
};

export function PrincipalPortalNav({ authz }: Props) {
  const pathname = usePathname();

  return (
    <nav
      aria-label="Principal portal"
      className="flex flex-wrap gap-1 border-b border-border pb-3"
    >
      {PRINCIPAL_PORTAL_NAV.map((item) => {
        const active =
          item.href === "/dashboard/principal"
            ? pathname === item.href
            : pathname === item.href || pathname.startsWith(`${item.href}/`);
        return (
          <Can key={item.id} permission={item.permission} bootstrap={authz}>
            <Link
              href={item.href}
              className={`rounded-md px-3 py-1.5 text-sm ${
                active
                  ? "bg-feezy-indigo/10 font-medium text-feezy-indigo"
                  : "text-muted hover:bg-surface hover:text-foreground"
              }`}
            >
              {item.label}
            </Link>
          </Can>
        );
      })}
    </nav>
  );
}
