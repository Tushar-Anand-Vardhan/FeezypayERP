...
> **Companions:** MASTER §59 · [`teacher-workspace.md`](teacher-workspace.md) · [`rbac.md`](rbac.md) · WF-TCH-* in [`daily-workflows.md`](../operations/daily-workflows.md)
...
| `/dashboard/teacher/marks` | `assessment.results.enter` | assessment |
| `/dashboard/teacher/homework` | `homework.read` / assign | homework |
| `/dashboard/teacher/behaviour` | `conduct.incident.record` | behaviour |
| `/dashboard/teacher/events` | `engagement.event.read` | events/calendar |
...
New teacher UI lives under `components/teacher-portal/` and calls `lib/{attendance,assessment,…}` actions. Do not add parallel OLTP writers.
