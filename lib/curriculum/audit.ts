...
export async function writeCurriculumAudit(
  ...
  const { error } = await supabase.from("curriculum_audit_log").insert({
  ...
  if (error) {
    console.error("curriculum_audit_log insert failed:", error.message);
  }
}

/** Pack-level create/update/archive/publish/clone → local audit + editing framework. */
export async function recordCurriculumPackMutation(
  supabase: Supabase,
  input: {
    schoolId: string;
  ...
  await writeCurriculumAudit(supabase, {
    ...
    entityType: input.entityType ?? "curriculum",
